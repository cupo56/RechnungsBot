"""
Gutschrift – GUI-Tab zur manuellen Erfassung und Erstellung von Gutschriften.
"""
...
from src.pdf.credit_note import generate_credit_note
...
class CreditNoteTab:
    """Haupt-Widget für den Gutschriften-Tab."""
    ...
    def __init__(self, parent):
        ...
        ttk.Label(hdr, text="Gutschriften", style="Title.TLabel").pack(anchor=tk.W)
        ttk.Label(
            hdr,
            text="Gutschriften zu Rechnungen erstellen — Positionen manuell eintragen",
            style="Sub.TLabel",
        ).pack(anchor=tk.W)
        ...
        self.var_nr   = tk.StringVar(value=cfg.get("last_credit_note_number", ""))
        self.var_date = tk.StringVar(value=datetime.date.today().strftime("%d.%m.%Y"))

        ttk.Label(settings, text="Rechnung Nr. (Gutschrift zu):").grid(row=0, column=0, sticky=tk.W, pady=3)
        ...
        self.var_ust_enabled = tk.BooleanVar(value=cfg.get("default_credit_note_ust_enabled", True))
        self.var_ust_percent = tk.StringVar(value=str(cfg.get("default_credit_note_ust_percent", 20.0)))
        ...
        self.var_girocode_enabled = tk.BooleanVar(value=cfg.get("default_credit_note_girocode_enabled", True))
        ...
        recipient = cfg.get("last_credit_note_recipient", {})
        ...
        self.var_cust_phone   = tk.StringVar(value=recipient.get("phone", ""))
        ...
        cust_row("Telefon:", self.var_cust_phone, 5)
        cust_row("UID-Nr.:", self.var_cust_vat, 6)
    ...
    def _add_item(self):
        ...
        if ref and not ref.lower().startswith("rechnung"):
            ref = f"Rechnung {ref}"
    ...
    def _delete_item(self, row_id):
        ...
        label = item["description"] or item["reference"] or "diese Position"
        if not messagebox.askyesno("Position entfernen",
                                    f"Soll die Position „{label}“ wirklich entfernt werden?"):
            return
    ...
    def _commit_cell_edit(self):
        ...
        if value_type is float:
            ...
            if field_key == "reference" and raw and not raw.lower().startswith("rechnung"):
                raw = f"Rechnung {raw}"
    ...
    def _update_template_combobox(self):
        names = list(self.config.get("credit_note_customer_templates", {}).keys())
    ...
    def _on_template_selected(self, _=None):
        ...
        tpl  = self.config.get("credit_note_customer_templates", {}).get(name, {})
        ...
        self.var_cust_phone.set(tpl.get("phone", ""))
    ...
    def _save_template(self):
        ...
        templates = self.config.setdefault("credit_note_customer_templates", {})
        templates[tpl_name] = {
            ...
            "phone":    self.var_cust_phone.get().strip(),
            "vat":      self.var_cust_vat.get().strip(),
        }
        save_config({"credit_note_customer_templates": templates})
        self._update_template_combobox()
    ...
    def _delete_template(self):
        ...
        if messagebox.askyesno("Löschen", f"Vorlage '{name}' wirklich löschen?"):
            templates = self.config.get("credit_note_customer_templates", {})
    ...
    def _create_invoice(self):
        ...
        if not invoice_nr:
            messagebox.showwarning("Rechnungsnummer fehlt", "Bitte die Rechnungsnummer eingeben, zu der die Gutschrift gehört.")
            return

        invoice_date = self.var_date.get().strip()
        if not invoice_date:
            messagebox.showwarning("Datum fehlt", "Bitte ein Datum eingeben.")
            return
        ...
        default_name = f"Gutschrift_{invoice_nr.replace('/', '_')}.pdf"
        output_path = filedialog.asksaveasfilename(
            title="Gutschrift speichern als",
        ...
        self.status_label.configure(text="Gutschrift wird erstellt…", foreground=_MUTED)
        self.btn_create.configure(state="disabled", bg=_BLUE_DIS, cursor="arrow")

        def _thread():
            try:
                generate_credit_note(items, invoice_data, customer_data, output_path)
    ...
    def _on_pdf_complete(self, output_path, invoice_nr, customer_data):
        self.btn_create.configure(state="normal", bg=_BLUE, cursor="hand2")
        msg = f"Gutschrift gespeichert: {os.path.basename(output_path)}"
        self.status_label.configure(text=msg, foreground=_GREEN)
        self._save_current_config(invoice_nr, customer_data)
    ...
    def _on_pdf_error(self, error_msg):
        ...
        messagebox.showerror("Fehler", f"Fehler bei der Gutschrift-Erstellung:\n\n{error_msg}")
    ...
    def _save_current_config(self, invoice_nr, customer_data):
        try:
        ...
        save_config({
            "last_credit_note_number":              invoice_nr,
            "default_credit_note_ust_enabled":      self.var_ust_enabled.get(),
            "default_credit_note_ust_percent":      ust_pct,
            "default_credit_note_girocode_enabled": self.var_girocode_enabled.get(),
            "last_credit_note_recipient":           dict(customer_data),
            "credit_note_customer_templates":       self.config.get("credit_note_customer_templates", {}),
        })

    def _save_to_db(self, pdf_path, invoice_nr, customer_data):
        """Speichert die Gutschrift nicht-blockierend in der Datenbank."""
        cfg = load_config()
        ...
        def _thread():
            success, msg = db.save_invoice(
                ...
                item_count=len(self._items), doc_type="gutschrift",
            )
            if success:
