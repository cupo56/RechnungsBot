"""
PDF-Gutschriftsgenerator für RechnungsBot.
Erstellt professionelle PDF-Gutschriften im Layout der Handelsagentur Adis Sefer.
"""
...
# Tabellen-Spalten (X-Positionen) — gutschriftspezifisch
...
class CreditNoteGenerator:
    ...
    def __init__(self, items, invoice_data, customer_data, output_path):
        ...
        Args:
            ...
            customer_data: dict mit name, street, plz_city, country, phone, vat
    ...
    def generate(self):
        """Erstellt die PDF-Gutschrift und speichert sie."""
        register_fonts()

        self.c = canvas.Canvas(self.output_path, pagesize=A4)
        self.c.setTitle(f"Gutschrift {self.inv['number']}")
    ...
    def _calculate_header_height(self):
        ...
        h = 4.5 + 6 * 4.0 + 2.0
        h += 4.5 + 4.0
        for field in ("street", "plz_city", "country", "phone"):
    ...
    def _draw_page_header(self, total_pages=1):
        ...
        if vat.strip():
            c.drawString(MARGIN_LEFT, y, f"UID {vat}")
            y -= 4 * mm

        y -= 18 * mm

        # --- Gutschriftstitel & Datum ---
        c.setFont("Arial-Bold", FONT_SIZE_TITLE)
        c.drawString(MARGIN_LEFT, y, f"Gutschrift zu den Rechnung  Nr.{self.inv['number']}")
        c.setFont("Arial", FONT_SIZE_NORMAL)
    ...
    def _draw_girocode_in_footer(self, footer_y):
        ...
        try:
            ...
            reference = f"Gutschrift {self.inv['number']}"
...
def generate_credit_note(items, invoice_data, customer_data, output_path):
    """
    Erstellt eine PDF-Gutschrift.
    ...
    """
    generator = CreditNoteGenerator(items, invoice_data, customer_data, output_path)
